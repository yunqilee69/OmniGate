...
### Do
- **Use Ant Design components exclusively** — the app UI is built with AntD (React); every control (button, select, input, table, modal, tabs) must come from `antd`. Only when AntD genuinely lacks a widget may a custom component be written, and it must consume this spec's tokens (colors / radii / hairlines / typography).
...
### Don't
- **Don't use native HTML form elements** (`<select>`, `<input>`, `<textarea>`, `<button>`) in app pages — they bypass the AntD theme (border, radius, font tokens) and produce untyped values (e.g. select values are always strings) that break API contracts. Always use the AntD equivalent.
